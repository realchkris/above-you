<template>
	<transition name="fade">
		...
		<div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
			<div class="base-container bg-ay-purple text-white w-full max-w-sm p-6 rounded-lg shadow-lg space-y-4 text-center">
				...
				<transition name="fade" mode="out-in">
					...
					<div v-else class="flex justify-center gap-4 mt-4">
						<button @click="$emit('cancel')" class="primary-button bg-ay-lavender hover:bg-ay-purple">
							Cancel
						</button>
						<button @click="$emit('confirm')" class="primary-button bg-red-600 hover:bg-red-700">
							Confirm
						</button>
					</div>
				</transition>
